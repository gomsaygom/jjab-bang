    ...
    <div style={{ minHeight:'100vh', background:'#f8fafc' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 24px', background:'#fff', borderBottom:'1px solid #e2e8f0' }}>
        ...
        <div style={{ display:'flex', gap:12, alignItems:'center' }}>
          ...
          <button onClick={logout} style={{ padding:'6px 12px', cursor:'pointer', borderRadius:6, border:'1px solid #ddd', background:'#fff' }}>로그아웃</button>
        </div>
      </div>

      <div style={{ maxWidth:800, margin:'0 auto', padding:'32px 24px' }}>
        {/* 프로필 카드 */}
        <div style={{ background:'#fff', borderRadius:16, padding:'24px', border:'1px solid #e2e8f0', marginBottom:24, display:'flex', alignItems:'center', gap:20 }}>
          <div style={{ width:64, height:64, borderRadius:'50%', background:'#4F8EF7', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>
            👤
          </div>
          <div>
            <h3 style={{ margin:'0 0 4px', fontSize:20 }}>{user?.nickname}</h3>
          ...
          <div>
            {loading ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {favorites.map(f => (
                  <div key={f.property_id} style={{ background:'#fff', borderRadius:12, padding:'20px', border:'1px solid #e2e8f0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div>
                      <h4 style={{ margin:'0 0 6px', fontSize:16 }}>{f.name}</h4>
                      <p style={{ margin:'0 0 4px', fontSize:13, color:'#64748b' }}>
                        {f.district} · {f.type} · {f.deal_type}
                      </p>
                      <p style={{ margin:0, fontSize:15, fontWeight:700, color:'#4F8EF7' }}>
                        {f.price ? (f.price / 10000).toFixed(1) + '억' : '-'}
                      </p>
                    </div>
                    <div style={{ display:'flex', gap:8 }}>
                      <button onClick={() => navigate(`/property/${f.property_id}`)}
                        style={{ padding:'8px 16px', borderRadius:8, border:'1px solid #e2e8f0', cursor:'pointer', fontSize:13, background:'#fff' }}>
                        상세보기
                      </button>
                      <button onClick={() => removeFavorite(f.property_id)}
                        style={{ padding:'8px 16px', borderRadius:8, border:'none', cursor:'pointer', fontSize:13, background:'#fee2e2', color:'#dc2626' }}>
                        삭제
                      </button>
                    </div>
                  </div>
                ...
                {reviews.map(r => (
                  <div key={r.id} style={{ background:'#fff', borderRadius:12, padding:'20px', border:'1px solid #e2e8f0' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                      <div>
                        <h4 style={{ margin:'0 0 4px', fontSize:15 }}>{r.property_name || '매물'}</h4>
                        <p style={{ margin:0, fontSize:12, color:'#94a3b8' }}>{r.created_at?.slice(0,10)}</p>
                      </div>
                      <button onClick={() => deleteReview(r.id)}
                        style={{ padding:'6px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:12, background:'#fee2e2', color:'#dc2626' }}>
                        삭제
                      </button>
                    </div>
