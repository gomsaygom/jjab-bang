...
export default function Chart() {
  ...
  const options = {
    ...
    plugins: {
      legend: { position: 'top' },
      tooltip: { callbacks: { label: ctx => `평균 보증금: ${ctx.raw}억원` } }
    },
    scales: {
      y: { ticks: { callback: v => `${v}억` }, grid: { color: '#f1f5f9' } },
      x: { grid: { color: '#f1f5f9' } }
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#f8fafc' }}>
      ...
      <div style={{ maxWidth:960, margin:'0 auto', padding:'32px 16px' }}>
        ...
        <div style={{ background:'#fff', borderRadius:16, padding:'20px', border:'1px solid #e2e8f0', marginBottom:24 }}>
          <h3 style={{ margin:'0 0 20px', fontSize:15, color:'#1e293b' }}>
        ...
        {chartData.length > 0 && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
            {[
              ...
              <div key={card.label} style={{ background:'#fff', borderRadius:12, padding:'16px', border:'1px solid #e2e8f0', textAlign:'center' }}>
                <p style={{ fontSize:12, color:'#64748b', margin:'0 0 8px' }}>{card.label}</p>
                <p style={{ fontSize:'clamp(16px,4vw,22px)', fontWeight:700, color:'#4F8EF7', margin:0 }}>{card.value}</p>
              </div>
            ))}
          </div>
        )}
